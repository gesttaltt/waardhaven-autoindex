...
class TestStrategyService:
    ...
    @pytest.mark.unit
    def test_compute_index_no_delete_on_failure(
        self, test_db, sample_assets, sample_prices
    ):
        ...
        with patch("app.services.strategy.calculate_momentum_scores") as mock_momentum:
            ...
            remaining = (
                test_db.query(IndexValue)
                .filter(IndexValue.date == date(2023, 12, 31))
                .first()
            )
    ...
    def test_upsert_index_values(self, test_db, sample_assets, sample_prices):
        ...
        with patch("app.services.strategy.calculate_momentum_scores") as mock_momentum:
            ...
            with patch(
                "app.services.strategy.calculate_risk_parity_weights"
            ) as mock_risk:
        ...
        values = (
            test_db.query(IndexValue).filter(IndexValue.date == date(2024, 1, 1)).all()
        )
        assert len(values) == 1
    ...
    @pytest.mark.unit
    def test_backup_creation_on_update(
        self, test_db, sample_assets, sample_prices, caplog
    ):
        ...
        existing_allocation = Allocation(
            date=date(2024, 1, 1), asset_id=sample_assets[0].id, weight=0.25
        )
        test_db.add(existing_value)
...
class TestDataIntegrity:
    ...
    def test_old_data_cleanup(self, test_db, sample_assets):
        ...
        for asset in sample_assets:
            price = Price(
                asset_id=asset.id, date=recent_date, close=100.0, volume=1000000
            )
            test_db.add(price)
        test_db.commit()
        ...
        old_remaining = (
            test_db.query(IndexValue).filter(IndexValue.date == old_date).first()
        )
        assert old_remaining is None

        # Recent data should remain
        recent_remaining = (
            test_db.query(IndexValue).filter(IndexValue.date == recent_date).first()
        )
