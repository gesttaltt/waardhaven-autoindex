...
class TestIndexEndpoints:
    ...
    def test_get_index_history(self, client, auth_headers, test_db):
        ...
        for i in range(5):
            test_db.add(IndexValue(date=date(2024, 1, i + 1), value=100 + i))
        test_db.commit()

        response = client.get("/api/v1/index/history", headers=auth_headers)
    ...
    @pytest.mark.api
    def test_get_current_allocations(
        self, client, auth_headers, test_db, sample_assets
    ):
        """Test getting current allocations."""
        # Add allocations
        latest_date = date(2024, 1, 1)
        for i, asset in enumerate(sample_assets[:3]):
            test_db.add(Allocation(date=latest_date, asset_id=asset.id, weight=0.33))
        test_db.commit()

        response = client.get("/api/v1/index/current", headers=auth_headers)
    ...
    def test_simulate_investment(self, client, auth_headers, test_db):
        ...
        for i, val in enumerate(values):
            test_db.add(IndexValue(date=date(2024, 1, i + 1), value=val))
        test_db.commit()

        response = client.post(
            "/api/v1/index/simulate",
            headers=auth_headers,
            json={"amount": 10000, "currency": "USD", "start_date": "2024-01-01"},
        )
    ...
    def test_get_asset_history(self, client, auth_headers, test_db, sample_assets):
        ...
        response = client.get("/api/v1/index/assets/AAPL/history", headers=auth_headers)
...
class TestStrategyEndpoints:
    ...
    def test_get_strategy_config(self, client, auth_headers, sample_strategy_config):
        ...
        response = client.get("/api/v1/strategy/config", headers=auth_headers)
    ...
    def test_update_strategy_config(self, client, auth_headers, sample_strategy_config):
        ...
        total = (
            data["momentum_weight"]
            + data["market_cap_weight"]
            + data["risk_parity_weight"]
        )
    ...
    def test_get_risk_metrics(self, client, auth_headers, test_db):
        ...
        response = client.get("/api/v1/strategy/risk-metrics", headers=auth_headers)
...
class TestDiagnosticsEndpoints:
    ...
    def test_data_status(
        self, client, auth_headers, test_db, sample_assets, sample_prices
    ):
        """Test data status endpoint."""
        response = client.get("/api/v1/diagnostics/data-status", headers=auth_headers)
...
class TestManualRefreshEndpoints:
    ...
    def test_trigger_refresh_with_auth(self, client, auth_headers, test_db):
        ...
        with patch("app.routers.manual_refresh.refresh_all") as mock_refresh:
            ...
            response = client.post(
                "/api/v1/manual/refresh", headers=auth_headers, json={"mode": "smart"}
            )
...
class TestBenchmarkEndpoints:
    ...
    def test_get_sp500_benchmark(self, client, auth_headers, test_db):
        ...
        response = client.get("/api/v1/benchmark/sp500", headers=auth_headers)
