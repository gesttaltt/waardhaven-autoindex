...
class TestTwelveDataProvider:
    ...
    def test_fetch_historical_prices_with_cache(self, provider, mock_redis):
        ...
        cached_data = pd.DataFrame({
            'Close': [150.0, 151.0],
            'Open': [149.0, 150.5]
        }, index=['2024-01-01', '2024-01-02']).to_json()
    ...
    def test_fetch_historical_prices_batch(self, provider):
        ...
        batch_data = {
            'AAPL': {
                'values': [
                    {'datetime': '2024-01-01', 'close': 150.0, 'open': 149.0, 
                     'high': 151.0, 'low': 148.5, 'volume': 1000000}
                ]
            },
            'MSFT': {
                'values': [
                    {'datetime': '2024-01-01', 'close': 380.0, 'open': 378.0,
                     'high': 381.0, 'low': 377.0, 'volume': 2000000}
                ]
            }
        }
    ...
    def test_health_check(self, provider):
        ...
        mock_usage.as_json.return_value = {
            'current_usage': 100,
            'plan_limit': 800
        }
