...
def compute_index(self, strategy_config: Optional[Dict] = None, db=None) -> Dict[str, Any]:
    """
...
def generate_report(self, report_type: str = "performance", period_days: int = 30, db=None) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if report_type == "performance":
            ...
            index_values = db.query(IndexValue).filter(
            ...
            latest_date = db.query(Allocation.date).order_by(Allocation.date.desc()).first()
            if latest_date:
                allocations = db.query(Allocation).filter(
                    Allocation.date == latest_date[0]
                ).all()
                
                report_data["allocations"] = [
                    {"asset_id": a.asset_id, "weight": a.weight}
            ...
            latest_risk = db.query(RiskMetrics).order_by(
                RiskMetrics.date.desc()
            ).first()
...
def cleanup_old_data(self, days_to_keep: int = 365, db=None) -> Dict[str, Any]:
    ...
    try:
        ...
        old_index_values = db.query(IndexValue).filter(IndexValue.date < cutoff_date).count()
        old_allocations = db.query(Allocation).filter(Allocation.date < cutoff_date).count()
