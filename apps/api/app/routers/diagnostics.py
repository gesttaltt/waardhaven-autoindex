...
def check_database_status(db: Session = Depends(get_db)):
    ...
    try:
        status = {"timestamp": datetime.utcnow().isoformat(), "tables": {}}
...
def test_refresh_process(db: Session = Depends(get_db)):
    ...
    try:
        ...
        results["steps"].append(
            {"step": "ensure_assets", "status": "success", "asset_count": asset_count}
        )
        ...
        try:
            ...
            results["steps"].append(
                {"step": "database_write", "status": "failed", "error": str(e)}
            )
...
def recalculate_autoindex(db: Session = Depends(get_db)):
    ...
    try:
        ...
        result = {"timestamp": datetime.utcnow().isoformat(), "status": "starting"}

        # Get counts before
        before_index_count = db.query(func.count()).select_from(IndexValue).scalar()
        before_allocation_count = (
            db.query(func.count()).select_from(Allocation).scalar()
        )
        ...
        # Get sample values
        sample_values = (
            db.query(IndexValue).order_by(IndexValue.date.desc()).limit(5).all()
        )
...
def get_twelvedata_status(
    ...
    try:
        ...
        rate_limit_info = {
            ...
            "credits_available": service.rate_limiter.credits_per_minute
            - len(service.rate_limiter.credits_used),
        }
