...
class NewsService:
    ...
    def search_news(
        ...
        if sentiment_min is not None or sentiment_max is not None:
            ...
            if sentiment_min is not None:
                query = query.filter(NewsSentimentModel.sentiment_score >= sentiment_min)
            if sentiment_max is not None:
                query = query.filter(NewsSentimentModel.sentiment_score <= sentiment_max)
    ...
    def get_article(self, article_id: str) -> Optional[Dict[str, Any]]:
        ...
        article = self.db.query(NewsArticleModel).filter(
            NewsArticleModel.external_id == article_id
        ).first()
    ...
    def get_similar_articles(
        ...
        try:
            ...
            base_article = self.db.query(NewsArticleModel).filter(
                NewsArticleModel.external_id == article_id
            ).first()
            ...
            if entity_symbols:
                similar_articles = self.db.query(NewsArticleModel).join(
                    NewsEntityModel
                ).filter(
    ...
    def get_entity_sentiment(
        ...
        recent_articles = self.db.query(NewsArticleModel).join(
            NewsEntityModel
        ).filter(
        ...
        if recent_articles:
            sentiments = [a.sentiment.sentiment_score for a in recent_articles if a.sentiment]
        ...
        average_sentiment = sum(h.sentiment_score * h.article_count for h in history) / max(total_articles, 1)
        
        # Get top sources
        top_sources = self.db.query(
            NewsSourceModel.name,
            func.count(NewsArticleModel.id).label('count')
        ).join(
            NewsArticleModel
        ).join(
            NewsEntityModel
    ...
    def get_trending_entities(
        ...
        query = self.db.query(
            ...
            NewsEntityModel.created_at >= datetime.now() - timedelta(days=7)
        )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        ).group_by(NewsSentimentModel.sentiment_label).all()
        
        # Recent activity
        recent_count = self.db.query(func.count(NewsArticleModel.id)).filter(
            NewsArticleModel.published_at >= datetime.now() - timedelta(days=1)
        ).scalar()
    ...
    def _store_article(self, article_data) -> Optional[NewsArticleModel]:
        ...
        existing = self.db.query(NewsArticleModel).filter(
            NewsArticleModel.external_id == article_data.uuid
        ).first()
            ...
            for entity_data in article_data.entities:
                ...
                if entity_data.symbol:
                    asset = self.db.query(Asset).filter(
                        Asset.symbol == entity_data.symbol
                    ).first()
    ...
    def _calculate_sentiment_history(
        ...
        articles = self.db.query(NewsArticleModel).join(
            NewsEntityModel
        ).filter(
            NewsEntityModel.symbol == symbol,
            NewsArticleModel.published_at >= start_date,
    ...
    def _article_to_dict(self, article: Optional[NewsArticleModel]) -> Optional[Dict[str, Any]]:
