...
class TwelveDataService:
    ...
    def fetch_prices(self, symbols: List[str], start: date, end: Optional[date] = None) -> pd.DataFrame:
        """
    ...
    def _process_price_data(self, df: pd.DataFrame, symbol: str) -> pd.DataFrame:
        ...
        if len(df) > 1:
            ...
            if len(extreme_returns) > 0:
                logger.warning(f"{symbol}: {len(extreme_returns)} extreme movements (>50%)")
...
def fetch_prices(symbols: List[str], start: date, end: Optional[date] = None) -> pd.DataFrame:
